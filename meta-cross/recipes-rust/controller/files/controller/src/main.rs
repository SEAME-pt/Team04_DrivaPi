...
fn run_manual_mode(
    ...
    match serialport::new("/dev/ttyACM0", 115200)
        .timeout(Duration::from_millis(1000)).open(){
            Ok(p) => port = Some(p),
            Err(_) => println!("UART disabled, continuing without it"),
        }
// =================================================================================
...
// =================================================================================
    ...
    // =================================================================================
    //                          CHANGING AMBULANCE MODE
        if let Some(port) = &mut port {

            if input.button_a && !prev_a_button {
                ambulance_mode = !ambulance_mode;
                if ambulance_mode {
                    let _ = port.write_all(b"A\n");
                    println!("AMBULANCE MODE: {ambulance_mode}");
                } else {
                    let _ = port.write_all(b"V\n");
                    println!("AMBULANCE MODE: {ambulance_mode}");
                }

            }
            prev_a_button = input.button_a;
        }
