...
def emergency_trigger(mode, now, bidding_end, last_t, my_rssi, l_state):
    if mode == "EVP_EVAL" and now > bidding_end:
        ...
        if my_rssi > -999:
            return set_lights("G"), mode, last_t
        else:
            if l_state == "G":
                return  set_lights("Y"), mode, last_t
            else:
                set_lights("R")
                return set_lights("R"), mode, last_t
    return l_state, mode, last_t

def semaphore_logic(now, last_t, l_state, mode):
    if mode == "NORMAL":
        elapsed = time.ticks_diff(now, last_t)
        if l_state == "R" and elapsed > R_TIME:
            return set_lights("G"), now
        elif l_state == "G" and elapsed > G_TIME:
            set_lights("Y")
            return set_lights("Y"), now
        elif l_state == "Y" and elapsed > Y_TIME:
            set_lights("R")
            return set_lights("R"), now
    return l_state, last_t
...
def main():
    ...
    while True:
        ...
        l_state, mode, last_t = emergency_trigger(mode, now, bidding_end, last_t, my_rssi, l_state)
        ...
        # semaphore logic for normal operation
        l_state, last_t = semaphore_logic(now, last_t, l_state, mode)
