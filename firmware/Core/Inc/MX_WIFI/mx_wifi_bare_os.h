...
#define LOCK_INIT(A)                                      \
  tx_mutex_create(&(A), "mx_lock", TX_NO_INHERIT)

#define LOCK_DEINIT(A)                                    \
  tx_mutex_delete(&(A))

#define LOCK(A)                                           \
  tx_mutex_get(&(A), TX_WAIT_FOREVER)
...
#define SEM_INIT(A, COUNT)                               \
  tx_semaphore_create(&(A), "mx_sem", (COUNT))

#define SEM_DEINIT(A)                                    \
  tx_semaphore_delete(&(A))
...
#define FIFO_OK        \
  0
#define DELAY_MS(N)    \
  tx_thread_sleep(((N) * TX_TIMER_TICKS_PER_SECOND) / 1000)
