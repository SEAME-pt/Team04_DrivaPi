...
OUT_C="$ROOT_DIR/firmware/Core/Src/sensors_module_image.c"
...
"$CC" -nostdlib -Wl,--gc-sections -Wl,-Map,"$BUILD_DIR/sensors_module.map" \
  -T"$MODULE_DIR/sensors_module.ld" \
  -o "$BUILD_DIR/sensors_module.elf" \
  "$BUILD_DIR"/*.o

"$NM" "$BUILD_DIR/sensors_module.elf" > "$BUILD_DIR/sensors_module.nm"
"$OBJCOPY" -O binary "$BUILD_DIR/sensors_module.elf" "$BUILD_DIR/sensors_module.bin"

python3 - <<'PY' "$BUILD_DIR/sensors_module.bin" "$OUT_C"
import pathlib
import sys
...
lines = []
lines.append('#include "sensors_module_image.h"')
lines.append('')
lines.append('__attribute__((aligned(32)))')
lines.append('const UCHAR g_sensors_module_image[] = {')
...
lines.append('};')
lines.append('')
lines.append(f'const ULONG g_sensors_module_image_size = {len(raw)}u;')
out_path.write_text('\n'.join(lines) + '\n', encoding='ascii')
PY

echo "Built sensors module image: $OUT_C"