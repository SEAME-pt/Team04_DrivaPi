...
OUT_C="$ROOT_DIR/firmware/Core/Src/servo_motor_module_image.c"
...
MODULE_SRCS=(
  "$MODULE_DIR/servo_motor_module.c"
...
"$CC" -nostdlib -Wl,--gc-sections -Wl,-Map,"$BUILD_DIR/servo_motor_module.map" \
  -T"$MODULE_DIR/servo_motor_module.ld" \
  -o "$BUILD_DIR/servo_motor_module.elf" \
  "$BUILD_DIR"/*.o

"$NM" "$BUILD_DIR/servo_motor_module.elf" > "$BUILD_DIR/servo_motor_module.nm"
"$OBJCOPY" -O binary "$BUILD_DIR/servo_motor_module.elf" "$BUILD_DIR/servo_motor_module.bin"

python3 - <<'PY' "$BUILD_DIR/servo_motor_module.bin" "$OUT_C"
import pathlib
import sys
...
lines = []
lines.append('#include "servo_motor_module_image.h"')
lines.append('')
lines.append('__attribute__((aligned(32)))')
lines.append('const UCHAR g_servo_motor_module_image[] = {')
...
lines.append('};')
lines.append('')
lines.append(f'const ULONG g_servo_motor_module_image_size = {len(raw)}u;')
out_path.write_text('\n'.join(lines) + '\n', encoding='ascii')
PY

echo "Built servo motor module image: $OUT_C"
